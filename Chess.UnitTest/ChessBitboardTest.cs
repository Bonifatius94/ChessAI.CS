...
namespace Chess.UnitTest
{
    public class ChessBitboardTest : TestBase
    {
        ...
        public void ConstructorTest()
        {
            ...
            var bitboard = new ChessBitboard(board);
            ...
            for (int row = 0; row < 8; row++)
            {
                for (int column = 0; column < 8; column++)
                {
                    ...
                    var pieceAtPos = bitboard.GetPieceAt(pos);
                    Assert.True((bitboard.IsCapturedAt(pos) && pieces.Any(x => x.Piece == pieceAtPos)) || (!bitboard.IsCapturedAt(pos) && !pieces.Any(x => x.Position == pos)));
                }
            }
        }
    }
